...
namespace AddingIntApplication
{
    class AddingInt
    {
        ...
        {/* enter the two digits*/
            ...
            {
                if (Char.IsDigit(num1[num1.Length - 1 - i])) //to check if the given char is digit or not
                    n1[i] = num1[num1.Length - 1 - i] - 48;
                else
                {
                    Console.WriteLine("num1 invalid number");
                    Console.ReadKey();
                    return;
                }
            }
            ...
            {
                if (Char.IsDigit(num2[num2.Length - 1 - i])) //to check if the given char is digit or not
                    n2[i] = num2[num2.Length - 1 - i] - 48;
                else
                {
                    Console.WriteLine("num2 invalid number");
                    Console.ReadKey();
                    return;
                }
